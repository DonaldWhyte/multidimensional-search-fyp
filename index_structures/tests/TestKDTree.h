...
namespace mdsearch { namespace tests
{
	...
	TEST_F(KDTreeTests, Stats)
	{
		...
		KDTreeStats stats = structure.computeStats();
		EXPECT_EQ(4.0, stats.balanceFactor);
		EXPECT_EQ(5, stats.maxPathLength);

		const Real vals[] = { 6, 5.5, 2 };
		Point newPoint(3, vals);
		ASSERT_TRUE(structure.insert(newPoint));
		stats = structure.computeStats();
		EXPECT_EQ(((5.0 + 4.0 + 4.0) / 3.0), stats.balanceFactor); // 4.33r
		EXPECT_EQ(5, stats.maxPathLength);
	}
