...
namespace mdsearch
{
	...
	private:
		...
		void insertToStructure(const Point& point, Bucket* bucket = NULL);
		Bucket* getContainingBucket(const Point& point);
		int getPointIndexInBucket(const Point& point, const Bucket* bucket) const;
		int hashPoint(const Point& myPoint);

		typedef SplayTree<int, Bucket> SplayTreeType;
