...
\begin{figure}
	...
	\caption{$kd$-Tree Operations}
	\label{fig:kd-tree}
\end{figure}

\section{Main Hypothesis}

Since there has been no published evaluations of the Pseudo-Pyramid Tree, I shall not be making any hypotheses about how well it performs. It is suspected that it will perform well on high-dimensional data, as it is a hash-based approach, but little is currently known about the structure's performance, so there is a little basis for such suspicion. Before starting implementation, the following hypothesis regarding the Pyrmid Tree and $kd-Tree$ was devised:
