...
% TODO: make real figures nicer

\begin{figure}
		\begin{center}
			\begin{subfloat}[Shadow Instability Forming in One 2D Slice Through the Astrophysics Dataset Over Time \cite{astrophysics-dataset}\label{fig:astrophysics}]{%
				\includegraphics[scale=0.25]{figures/ionisation_front_instabilities.pdf}
			}
			\end{subfloat}~
...
\begin{table}
	\centering
	\makebox[\textwidth][c]{%
	\begin{tabular}{|p{3.5cm}|p{4cm}|p{3.5cm}|p{4cm}|p{4cm}|}
		\hline
		\textbf{Operation List} & \textbf{Dataset(s)} & \textbf{Varying Parameter} & \textbf{Parameter Values}  \\
		\hline
		...
		Insert-Query-Delete & Astrophysics ($t = 100$) & - & - \\
		Random Operations & Astrophysics ($t = 100$) & - & - \\
		Insert-Query-Delete & Hurricane Isabel ($t = 24$) & - & - \\
		Random Operations & Hurricane Isabel ($t = 24$) & - & - \\
		Insert-Query-Delete & Armadillo Mesh & - & - \\
		Random Operations & Armadillo Mesh & - & - \\
		\hline
	\end{tabular}}%
