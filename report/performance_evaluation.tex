...
This section evaluates the baselines and index structures discussed in Section \ref{chap:chosen-structures-hypothesis} using their execution times on all the synthetic and real datasets. The performance is illustrated with plots and bar charts. Tables containing the exact times used to generate the plots is available in Appendix \ref{chap:performance-timings}.

On all execution time plots, only \texttt{insert} and \texttt{delete} operations will be plotted. Since both point insertion or deletion will have to perform a point query (insert has to check the point is not already stored), point query execution time will not be plotted. See Appendix \ref{chap:performance-timings} for point query times.
...
There is a relationship between bucket size and performance -- higher average bucket sizes are paired with longer execution times. Ideally, the number of buckets is equal to the number of points, known as \textit{perfect hashing}. Table \ref{tab:perf-bucket-stats} shows the average and maximum bucket size, as well as how many buckets there are, for the real datasets and uniformly distributed synthetic dataset.
...
Sequential scan incurs almost no overhead since only three extra pointers are used by the underlying resizeable array. The $kd$-tree also produces small smallest overhead. Each point incurs 8/16 bytes of overhead, since they are stored in a node that contains pointers to its two children. 
