...
In addition to testing performance with respect to dimensionality, the structures have been tested against data of varying size. A uniformly randomly generated dataset with 1,000,000 points has been created. The performance of the three operations has been timed for different sized datasets sampled from the 1,000,000 point dataset. Again, the three operations are timed using the insert-query-delete sequence of operations. The times are displayed in Table \ref{tab:perf1-sizevary} and Figures \ref{fig:perf1-allinsert-n}, \ref{fig:perf1-alldelete-n} and \ref{fig:perf1-allpquery-n}. Unlike the previous two tests, the number of operations \textit{varies} for each dataset, so the times given in the table and plots is the \textbf{average} time of an operation, not the \textit{total}. This allows a fair comparison between operation times of each structure to be made as $n$ increases.

By measuring the average execution time of an operation in this way, insight into each structure's performance behaviour can be gained. If there is a linear relationship between the average time and $n$, then it is shown a structure's performance decreases linearly with dataset size. If the average timing does not change at all, then dataset size does not affect a structure's performance.
